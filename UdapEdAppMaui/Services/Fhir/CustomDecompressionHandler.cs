...
public class CustomDecompressionHandler : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (response?.Content != null)
        {
            ...
            await SecureStorage.Default.SetAsync(UdapEdConstants.FhirClient.FhirCompressedSize, countingStream.BytesRead.ToString());
            await SecureStorage.Default.SetAsync(UdapEdConstants.FhirClient.FhirDecompressedSize, Encoding.UTF8.GetByteCount(decompressedContent).ToString());
